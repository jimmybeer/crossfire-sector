...
const FIXTURE_PATH := "res://docs/data-definition/fixtures/match_state.json"
...
func _ready() -> void:
	...
	input_bindings = UIInputMap.new().default_desktop_bindings()
...
func _build_preview_command(unit_state: Dictionary) -> Dictionary:
	match selected_action:
		"attack":
			var target: Dictionary = _find_opponent_unit()
			if target.is_empty():
				return _build_move_command(unit_state)
			return _build_attack_command(target)
		"melee":
			var melee_target: Dictionary = _find_opponent_unit()
			if melee_target.is_empty():
				return _build_move_command(unit_state)
			return _build_melee_command(melee_target)
		"first aid":
			var aid_target: Dictionary = _find_friendly_down_unit()
			if aid_target.is_empty():
				return _build_move_command(unit_state)
			return _build_first_aid_command(aid_target)
		_:
			return _build_move_command(unit_state)
