...
func validate(command_data: Dictionary, state: Dictionary, data_config: Dictionary) -> Dictionary:
    ...
    if not ALLOWED_TYPES.has(cmd_type):
        errors.append(_error("invalid_type", "Unsupported command type: %s" % cmd_type, "AQ-007"))
    ...
    var optional_rules: Dictionary = _optional_rules_with_defaults(state, data_config)
    ...
    return {"ok": ok, "errors": errors, "preview": preview}
...
func _gates(cmd_type: String, optional_rules: Dictionary, errors: Array) -> void:
    if (
        ...
        and not optional_rules.get("campaign", true)
    ):
        errors.append(
            _error("campaign_disabled", "Command not allowed when campaign mode disabled", "GR-040")
        )
...
func _validate_move(
    ...
    if unit_state.is_empty():
        errors.append(_error("missing_unit", "Move requires valid actor unit", "GR-017"))
        return {}
...
func _validate_attack(
    ...
    var actor_id: String = command_data.get("actor_unit_id", "")
    var target_unit_id: String = command_data.get(
        "target_unit_id",
        payload.get("target_unit_id", "")
    )
    var target_cell: Dictionary = payload.get("target_cell", {})
    if actor_id == "" or target_cell.is_empty() or target_unit_id == "":
        errors.append(
            _error(
                "missing_actor_target",
                "Attack requires actor, target unit, and target cell",
                "GR-019"
            )
        )
        return {}
    var move_preview: Dictionary = _validate_move(
        command_data, payload, board, terrain_blocks, occupied, state, data_config, errors, true
    )
    var final_tiles: Array = move_preview.get("reachable_tiles", [])
    var final_pos: Dictionary = (
        final_tiles.back() if final_tiles.size() > 0 else _unit_position(actor_id, state)
    )
    ...
    var los_result: Dictionary = _raycast_los(
        final_pos,
        target_cell,
        terrain_blocks,
        occupied,
    ...
    var los_ok: bool = los_result.get("clear", false)
    if not los_ok:
        errors.append(_error("los_blocked", "Line of sight blocked", "GR-031"))
    var cover: bool = los_result.get("cover", _has_cover(target_cell, terrain_blocks, occupied))
    return {
...
func _validate_melee(
    ...
    var final_pos: Dictionary = (
        final_tiles.back()
        if final_tiles.size() > 0
        else _unit_position(command_data.get("actor_unit_id", ""), state)
    )
...
func _validate_first_aid(
    ...
    var target_unit: String = command_data.get("target_unit_id", payload.get("target_unit_id", ""))
    if target_unit == "":
...
func _validate_reroll(
    command_data: Dictionary, payload: Dictionary, units_by_id: Dictionary, errors: Array
) -> void:
    if command_data.get("actor_unit_id", "") == "":
        errors.append(_error("missing_actor", "Reroll requires actor unit", "GR-045"))
...
func _validate_advantage(
    _command_data: Dictionary, optional_rules: Dictionary, errors: Array
) -> void:
...
func _validate_quick_start(
    ...
    for unit_id in unit_ids:
        ...
        var move_cmd: Dictionary = {"actor_unit_id": unit_id, "payload": {"path": path}}
...
## Private: Normalize optional rules with default-off posture.
func _optional_rules_with_defaults(state: Dictionary, data_config: Dictionary) -> Dictionary:
    var defaults: Dictionary = {"campaign": false, "events": false, "commander": false}
    var merged: Dictionary = defaults.duplicate(true)
    merged.merge(data_config.get("optional_rules", {}), true)
    merged.merge(state.get("optional_rules", {}), true)
    return merged
...
func _get_move_allowance(
    unit_state: Dictionary, state: Dictionary, data_config: Dictionary, half_move: bool
) -> int:
...
func _validate_path(
    path: Array, board: Dictionary, terrain_blocks: Dictionary, _occupied: Array, errors: Array
) -> void:
...
## Private: Raycast-style LoS with explicit ray list for DTO alignment.
func _raycast_los(
    start_cell: Dictionary,
    target_cell: Dictionary,
    terrain_blocks: Dictionary,
    ...
    var los: Dictionary = {"clear": true, "rays": []}
    var line: Array = _bresenham_line(start_cell, target_cell)
    for point in line:
        if not _is_within_bounds(point, board):
            los["clear"] = false
            los["rays"].append({"cell": point, "blocked": true})
            break
        var blocked: bool = _is_impassable(point, terrain_blocks) or _is_occupied(point, occupied)
        los["rays"].append({"cell": point, "blocked": blocked})
        if blocked:
            los["clear"] = false
            break
    los["cover"] = _has_cover(target_cell, terrain_blocks, occupied)
    return los
